...
-- Selecione os programadores com idades entre 25 e 30 anos:
SELECT * FROM Programadores
WHERE idade BETWEEN 25 AND 30;

-- Calcule a soma total das idades de todos os usuarios:
SELECT SUM(idade) AS SomaDasIdades FROM usuarios;

-- Selecione os programadores com salários entre 5000 e 7000:
SELECT * FROM Programadores
WHERE salario BETWEEN 5000 AND 7000;

-- Calcule a soma total dos salários de todos os programadores:
SELECT SUM(salario) AS SomaDosSalarios FROM Programadores;

-- Selecione os usuarios com ids entre 10 e 15:
SELECT * FROM usuarios
WHERE id BETWEEN 10 AND 15;

-- Calcule a soma total das horas trabalhadas por todos os programadores:
SELECT SUM(horas_trabalhadas) AS TotalHorasTrabalhadas
FROM Programadores;

-- Selecione os programadores com idade entre 10 e 18:
SELECT *FROM Programadores
WHERE bonus BETWEEN 1000 AND 2000;

-- Calcule a soma total das idades da tabela contatos:
SELECT SUM(idade) AS total_idade
FROM contatos;

-- Selecione os contatos com idades maiores que 18 mas menores que 30:
